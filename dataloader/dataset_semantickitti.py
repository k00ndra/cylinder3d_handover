...
class voxel_dataset(data.Dataset):
    def __init__(self, in_dataset, grid_size, rotate_aug=False, flip_aug=False, ignore_label=255, return_test=False,
                 fixed_volume_space=False, max_volume_space=[35, 35, 5], min_volume_space=[-10, -10, -3],
                 cut_mix=False):
        'Initialization'
    ...
    def __getitem__(self, index):
        ...
        if self.cut_mix:
            ...
            # TODO moved into function as newxyz may not be defined otherwise
        ...
        test = np.concatenate([xyz, sig.reshape(-1, 1), labels.reshape(-1, 1)], axis=1)
        data_tuple = (test, processed_label)
...
def collate_fn_BEV_tta(data):
    ...
    label2stack = np.stack([da2[1] for da1 in data for da2 in da1]).astype(int)
